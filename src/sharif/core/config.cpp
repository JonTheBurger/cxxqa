...
// local
#include <sharif/core/config.hpp>
#include <sharif/util/filesystem.hpp>
#include <sharif/util/log.hpp>
#include <sharif/util/ranges.hpp>
 ...
 */
auto get_config_dirs(std::string_view app = "sharif") -> std::vector<fs::path>
{
...
auto config_dir_validator(std::string_view app = "sharif") -> CLI::Validator
{
...
auto Config::from_cli(int argc, char** argv) -> Config
{
  ...
  cli.usage(fmt::format("{} {}", fmt::styled("USAGE:", fmt::emphasis::bold | fmt::fg(fmt::color::lime_green)), fmt::styled("sharif [OPTIONS] <SUBCOMMAND>", fmt::fg(fmt::color::cyan))));
  ...
  cli.set_config("-c,--config", "sharif.toml")->transform(config_dir_validator("sharif"));
  ...
  inspect->description("Inspect the sharif application for troubleshooting");
  ...
  try
  {
    ...
    if (inspect->parsed())
    {
      ...
      {
        for (const auto& path : get_config_dirs("sharif"))
        {
