...
#include <sharif/util/xml.hpp>

// namespace
namespace sharif::cppcheck {
...
template <>
struct quick_status_code_from_enum<sharif::cppcheck::Code> : quick_status_code_from_enum_defaults<sharif::cppcheck::Code> {
  ...
  {
    using enum sharif::cppcheck::Code;
    static const std::initializer_list<mapping> errors = {
      { .value = MISSING_ELEMENT_RESULTS, .message = "Report missing required element <results>", .code_mappings = { sharif::Code::bad_message } },
      { .value = UNRECOGNIZED_VERSION, .message = "<results> version attribute has an unrecognized value", .code_mappings = { sharif::Code::bad_message } },
      // { .value = sharif::Error::, .message = "", .code_mappings = { sharif::Code::bad_message } },
    };
    return errors;
  }
};
